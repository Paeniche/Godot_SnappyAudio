# TODO Direct user to credits file or something to provide back to original QuickAudio addon
@icon("./icon.svg")
extends Node
...
func _play_sound(sound: AudioStream, player, bus := "Master", autoplay := true):
	player.stream = sound
...
func play_sound(sound: AudioStream, bus := "Master", autoplay := true) -> AudioStreamPlayer:
...
func play_sound_2d(sound: AudioStream, bus := "Master", autoplay := true) -> AudioStreamPlayer2D:
...
# Note: Remember to set the global_position or reparent(new_parent, false)!
func play_sound_3d(sound: AudioStream, bus := "Master", autoplay := true) -> AudioStreamPlayer3D:

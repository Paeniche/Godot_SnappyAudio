...
func _ready():
	...
	if not is_instance_valid(song_1):
		print(
			(
				'QuickAudio Music Demo script is lacking a "song_1". Set it in the inspector on the node: {0}'
				. format([get_path()])
			)
		)
		valid = false
	if not is_instance_valid(song_2):
		print(
			(
				'QuickAudio Music Demo script is lacking a "song_2". Set it in the inspector on the node: {0}'
				. format([get_path()])
			)
		)
		valid = false
...
func _input(event):
	if (
		not is_instance_valid(song_1)
		or not is_instance_valid(song_2)
		or not is_instance_valid(music_player)
	):
		return
